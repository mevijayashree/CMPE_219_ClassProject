...
class IngredientSelectorDisplay {
    ...
    render() {
		...
		recipeName.title       = this.recipe.description; 
		...
		let wrongIngredients     = this.recipe.wrongIngredients; 
        ...
        for(let j = 0; j < remainingIngredients;) {
            let index = getRandomInt(wrongIngredients.length);
            let dummyIngredient = wrongIngredients[index];
        ...
        this.submitButton.addEventListener('click', () => {
            if(this.checkedIngredients > 4) {
                alert('You\'re using too many ingredients! Select upto 4 ingredients.');
            }
			else if (this.checkedIngredients < 1) {
				alert('You\'re using too less ingredients! Select at least 1 Ingredient.');
			}
            else {
